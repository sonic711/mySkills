...
## 使用原則
- 主報告維持快速閱讀骨架：快速結論、業務流程、系統交易與資料流、交易資料格式、SQL 與資料存取、異常與風險、未確認關鍵證據。
- 只有當目標具備特定特徵時，才追加對應 facet。
- facet 是條件式附錄，不是每份報告都必填。
- facet 應補維護需要的細節，不應把主報告重新變回冗長技術清單。
- 若偵測到特徵但證據不足，仍要建立該 facet，並把未確認部分標成 `Unknown`。
- facet 內的關鍵證據只列未確認或待補查部分；已確認的入口、SQL、client、設定檔證據應放在 facet 主體說明中，不集中輸出成證據清單。
