...
| `analysis_focus` | 否 | 用來判斷舊報告是否涵蓋需求，例如 `業務流程簡述`、`請求到回應`、`系統時序圖` |
...
- 報告標題、目標定位、主要用途與角色、業務流程簡述、未確認關鍵證據
...
| 策略 | 使用時機 | 行動 |
|------|----------|------|
| `Reuse` | 舊報告已涵蓋本次需求，且有「業務流程簡述」、「請求到回應完整說明」、「未確認關鍵證據」；跨系統時也已有「系統架構交易時序圖」；若有 pull diff，必須是 `NoImpact` | 引用既有報告摘要，不重讀完整程式 |
| `Patch` | 舊報告可用，但缺少新章節、證據格式過舊、使用者要求補特定面向，或 pull diff 命中目標/相關鏈路 | 只補缺口；若缺少「業務流程簡述」，用既有證據補業務面說明；若邏輯有改變，正式報告要補「本次邏輯變更」 |
...
|-----------|-----------|------|------|-------------|----------|----------|----------|----------|----------|------|
| | | Service / Controller / DAO / Job / Feature / Flow / Issue | uat / main | commit sha | 未分析 / 部分分析 / 已分析 / 需補章節 | | 是 / 否 | 用途、業務流程簡述、上下游、請求到回應、系統時序圖、異常流、依賴、未確認關鍵證據 | YYYY-MM-DD | |
```
...
- `已分析`：已有符合現行模板的正式報告。
- `需補章節`：已有報告，但缺少「業務流程簡述」、「請求到回應完整說明」、跨系統必備的「系統架構交易時序圖」、未確認關鍵證據、facet 或其他本次必備內容。
