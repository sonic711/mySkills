...
| `analysis_focus` | `用途` / `業務流程簡述` / `上下游` / `交易細節` / `依賴影響` / `跨專案比較` / `路由鏈` / `資料契約` / `請求到回應` / `異常流` / `流程圖` / `系統時序圖` / `實作細節` / `變數分析` / `方法分析` / `物件結構` / `完整流程` / `問題原因` / `寫入點` / `套件引用` / `邏輯分支` / `驗證方式` / `反證審查` / `精確度檢查` |
...
target_type: class
analysis_focus: 用途, 業務流程簡述, 上下游, 交易細節, 路由鏈, 資料契約, 請求到回應, 異常流, 流程圖
scope_hint: 請先用業務角度說明這支 service 解決什麼事，再說明誰呼叫它、它呼叫誰、交易資料怎麼流動
...
target_name: [類別名 / 功能名]
target_type: class
analysis_focus: 用途, 業務流程簡述, 上下游, 請求到回應, 異常流
scope_hint: 請先查 analysis_registry 與既有報告；若已有完整報告就重用，若缺少業務流程簡述等章節只補缺口
output_requirements: 繁體中文, analysis_output/<project_name>/, md
```
...
target_name: [類別名或檔名]
target_type: class
analysis_focus: 業務流程簡述, 實作細節, 變數分析, 方法分析, 物件結構, 完整流程, 請求到回應, 流程圖, 反證審查, 精確度檢查
scope_hint: 我不想看原始碼，請先用業務角度簡述它負責的流程，再完整拆解每個成員變數、每個方法、關鍵局部變數、物件結構與完整流程
output_requirements: 繁體中文, analysis_output/<project_name>/, md
```
...
target_type: flow
analysis_focus: 用途, 業務流程簡述, 上下游, 交易細節, 路由鏈, 資料契約, 請求到回應, 異常流, 流程圖
scope_hint: 請先用業務角度說明整體流程的目的、參與對象、輸入與結果，再分析涉及哪些核心服務、資料流、交易節點與錯誤處理
output_requirements: 繁體中文, analysis_output/<project_name>/, md
```
...
target_type: feature
analysis_focus: 用途, 業務流程簡述, 上下游, 交易細節, 依賴影響, 路由鏈, 資料契約, 請求到回應, 異常流, 流程圖
...
target_type: feature
analysis_focus: 用途, 業務流程簡述, 上下游, 交易細節, 依賴影響, 資料契約, 請求到回應, 流程圖
...
target_type: feature
analysis_focus: 跨專案比較, 用途, 業務流程簡述, 上下游, 交易細節, 依賴影響, 流程圖
...
target_name: [類別名 / 功能名]
target_type: class
analysis_focus: 用途, 業務流程簡述, 上下游, 交易細節, 路由鏈, 資料契約, 請求到回應, 異常流, 流程圖, 反證審查, 精確度檢查
...
analysis_focus: 業務流程簡述, 問題原因, 資料流, 寫入點, 套件引用, 邏輯分支, 驗證方式, 請求到回應, 反證審查, 精確度檢查
...
- 想完整拆解單一程式：`analysis_focus` 加 `業務流程簡述, 實作細節, 變數分析, 方法分析, 物件結構, 完整流程, 請求到回應`。
- 想讓不懂系統的人快速理解：`analysis_focus` 加 `業務流程簡述, 請求到回應`；前者用業務角度說明目的與結果，後者用白話說明系統從收到請求到回應結果的過程。
