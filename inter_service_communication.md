...
| `analysis_focus` | 否 | `用途` / `業務流程簡述` / `系統交易與資料流` / `資料格式` / `SQL與資料存取` / `上下游` / `交易細節` / `依賴影響` / `跨專案比較` / `路由鏈` / `資料契約` / `異常流` / `流程圖` / `系統時序圖` / `請求到回應` |
...
- 必須整理「系統交易與資料流」表格：順序、方向、來源、目的地、通訊方式、資料格式/物件、主要欄位、結果。
- 若資料格式不明，標為 `Unknown`，並指出需補查 DTO、layout、schema、mapper 或 log。
...
- 必須補一段「業務流程簡述」：用業務面向說明這個流程的目的、參與對象、輸入資料、處理重點與最後結果；避免在此段落展開 class、method、DTO、SQL。
- 必須補「主要處理邏輯」：用白話按時間順序說明收到請求、辨識路由、檢查參數、轉換資料、呼叫下游、組裝回應或送出通知，以及失敗時怎麼回應。
...
|------|------|------|--------|------|----------------|----------|------|
| 1 | inbound / outbound / MQ / DB / callback | | | API / gRPC / MQ / DB / file | JSON / XML / DTO / Map / event | | |
...
- [ ] 是否把鏈路順序寫清楚？
- [ ] 是否補上業務流程簡述，且不被技術細節淹沒？
- [ ] 是否補上系統交易與資料流，列明方向、來源、目的地、方式、資料格式、主要欄位與結果？
- [ ] 是否補上交易資料格式，列明輸入、輸出與必要欄位？
- [ ] 是否補上從接收請求到回應/通知完成的白話主要處理邏輯？
