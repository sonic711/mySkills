**Skill: Inter-Service Communication (跨服務通訊追蹤器) 分析報告**

**功能簡述:** `Inter-Service Communication` 負責重建 runtime 層級的呼叫鏈與資料流，追蹤服務間的通訊路徑。

**業務功能描述:** 協助開發者和維運人員理解分散式系統中服務間的實際通訊模式。本技能能夠分析 HTTP、gRPC、MQ 等多種通訊方式，重建交易的完整路徑，識別資料流向、同步/非同步鏈路，並標記潛在的通訊風險（如缺乏重試機制、契約不明等），從而支援系統的故障排除、性能優化和架構審查。

**關鍵字/標籤:** `Inter-Service Communication`, `跨服務通訊`, `呼叫鏈追蹤`, `資料流分析`, `分散式系統`, `HTTP`, `gRPC`, `MQ`, `通訊風險`, `交易追蹤`, `runtime分析`
...
- 若可辨識，記錄：
  ...
  - 出站 payload
  - 交易邊界與副作用
  - **具體的輸入資料範例 (例如：JSON/XML 結構)**
  - **具體的輸出資料範例 (例如：JSON/XML 結構)**
