...
| `analysis_focus` | 否 | `業務流程簡述` / `系統交易與資料流` / `資料格式` / `SQL與資料存取` / `問題原因` / `資料流` / `寫入點` / `套件引用` / `邏輯分支` / `驗證方式` / `請求到回應` |
...
- 若可定位入口或交易呼叫鏈，必須補「系統交易與資料流」與「交易資料格式」，讓讀者知道問題資料如何在系統間傳遞。
- 必須補「SQL 與資料存取」；有查詢/寫入/SP/mapper/repository 時列出，未發現也要明示。
...
## 5. 系統交易與資料流
| 順序 | 方向 | 來源 | 目的地 | 方式 | 資料格式/物件 | 問題欄位 | 結果 |
|------|------|------|--------|------|----------------|----------|------|
| 1 | inbound / outbound / DB / MQ / callback | | | API / method / MQ / DB / file | DTO / Entity / Map / SQL row / event | | |

## 6. 交易資料格式
| 欄位 | 來源 | 去向 | 型態/格式 | 目前看到的值/規則 | 問題點 |
|------|------|------|-----------|-------------------|--------|
| | | | | | |

## 7. SQL 與資料存取
| 類型 | 位置 | Table/SP/Mapper | SQL/方法摘要 | 條件 | 讀寫 | 與問題關係 |
|------|------|-----------------|--------------|------|------|------------|
...
- [ ] 是否補上系統交易與資料流、交易資料格式？
- [ ] 是否補上 SQL 與資料存取；沒有找到時是否明示？
- [ ] 是否追到欄位來源、中途轉換與寫入點？
- [ ] 若可定位入口，是否補上從接收請求到回應結果的白話主要處理邏輯？
