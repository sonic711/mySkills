...
| `analysis_focus` | 否 | `業務流程簡述` / `問題原因` / `資料流` / `寫入點` / `套件引用` / `邏輯分支` / `驗證方式` / `請求到回應` |
...
- 若可定位功能或交易，必須補「業務流程簡述」，用業務面向說明此問題發生在哪個業務段落、誰提供資料、系統應產生什麼結果，以及目前現象代表哪個業務結果不一致。
